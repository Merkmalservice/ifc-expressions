...
import {
    ...
    ExprContext,
    ExprListContext,
    FunctionCallContext, LiteralContext,
    MethodCallChainEndContext,
    MethodCallChainInnerContext,
    NumLiteralContext,
    SEAddSubContext, SEArrayExprContext,
    SEBooleanBinaryOpContext, SEComparisonContext,
...
import {EqualsExpr} from "./expression/comparison/EqualsExpr";
import {NotEqualsExpr} from "./expression/comparison/NotEqualsExpr";
import {Type} from "./parse/Types";
import {IfcExpressionValidationListener} from "./IfcExpressionValidationListener";
...
import {ComparisonOp} from "./expression/comparison/ComparisonOp";
import {NumericValue} from "./value/NumericValue";
import {GreaterThan} from "./expression/comparison/GreaterThan";
import {GreaterThanOrEqual} from "./expression/comparison/GreaterThanOrEqual";
import {LessThan} from "./expression/comparison/LessThan";
import {LessThanOrEqual} from "./expression/comparison/LessThanOrEqual";
...
export class ExprVisitor extends IfcExpressionVisitor<Expr<any>> {
    ...
    visitSEComparison: (ctx: SEComparisonContext) => Expr<any>=  ctx => {
        ...
        switch (ctx.CMP_OP().getText()) {
            case "==": return new EqualsExpr(left, right);
            case "!=": return new NotEqualsExpr(left, right);
            case ">": return new GreaterThan(left, right);
            case ">=": return new GreaterThanOrEqual(left, right);
            case "<": return new LessThan(left, right);
            case "<=": return new LessThanOrEqual(left, right);
        }
        throw new Error(`Parsing error: comparison operator '${ctx.CMP_OP().getText()}' not supported`);
    }
    ...
    private makePlusExpr(ctx: SEAddSubContext) {
        ...
        if (this.typeManager.isTypeOrBroader(Type.STRING, ctx._left, ctx._right)
            || this.typeManager.isTypeOrBroader(Type.NUMERIC, ctx._left, ctx._right)){
    ...
    visitSEBooleanBinaryOp: (ctx: SEBooleanBinaryOpContext) => Expr<any> = ctx => {
        switch (ctx._op.text) {
            case "&&":
                return new AndExpr(this.visit(ctx.singleExpr(0)), this.visit(ctx.singleExpr(1)));
            case "||":
                return new OrExpr(this.visit(ctx.singleExpr(0)), this.visit(ctx.singleExpr(1)));
            case "><":
                return new XorExpr(this.visit(ctx.singleExpr(0)), this.visit(ctx.singleExpr(1)));
        }
