...
import {ExprEvalError, ExprEvalTypeErrorObj} from "../ExprEvalResult";

export class PlusOrConcatExpr extends Expr2<NumericValue | StringValue, NumericValue | StringValue, NumericValue | StringValue> {
  ...
  calculateResult(
    ...
    if (leftResult instanceof NumericValue && rightResult instanceof NumericValue) {
      return NumericValue.of(leftResult.getValue().plus(rightResult.getValue()));
    } else if (leftResult instanceof StringValue && rightResult instanceof StringValue) {
